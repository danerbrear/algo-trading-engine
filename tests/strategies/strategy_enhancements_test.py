...
def mock_cache_manager(temp_cache_dir):
    ...
    def _mock_get_cache_dir(_self, *subdirs):
...
class TestVelocitySignalMomentumStrategyEnhancements:
    ...
    def setup_method(self):
        ...
        self.strategy = VelocitySignalMomentumStrategy(get_contract_list_for_date=self.get_contract_list_for_date, get_option_bar=self.get_option_bar, get_options_chain=self.get_options_chain)
        dates = pd.date_range(start='2024-01-01', periods=100, freq='D')
        self.sample_data = pd.DataFrame({'Close': [100 + i * 0.5 for i in range(100)], 'Open': [100 + i * 0.5 for i in range(100)], 'High': [101 + i * 0.5 for i in range(100)], 'Low': [99 + i * 0.5 for i in range(100)], 'Volume': [1000000] * 100}, index=dates)
    ...
    def test_check_backward_trend_success_upward_trend(self):
        ...
        upward_data = pd.DataFrame({'Close': [100 + i * 2 for i in range(50)], 'Open': [100 + i * 2 for i in range(50)], 'High': [101 + i * 2 for i in range(50)], 'Low': [99 + i * 2 for i in range(50)], 'Volume': [1000000] * 50}, index=dates)
        success, duration, return_val = self.strategy._check_backward_trend_success(upward_data, 20, 'up', min_duration=3, max_duration=10)
    ...
    def test_check_backward_trend_success_no_trend(self):
        ...
        dates = pd.date_range(start='2024-01-01', periods=50, freq='D')
        flat_data = pd.DataFrame({'Close': [100] * 50, 'Open': [100] * 50, 'High': [101] * 50, 'Low': [99] * 50, 'Volume': [1000000] * 50}, index=dates)
        success, duration, return_val = self.strategy._check_backward_trend_success(flat_data, 20, 'up', min_duration=3, max_duration=10)
    ...
    def test_check_trend_success_always_uses_backward(self):
        ...
        with patch.object(self.strategy, '_check_backward_trend_success') as mock_backward:
            ...
            result = self.strategy._check_trend_success(self.sample_data, 20, 'up', min_duration=3, max_duration=10)
            mock_backward.assert_called_once()
            call_args = mock_backward.call_args
            assert call_args[0][0].equals(self.sample_data)
...
class TestCreditSpreadStrategyEnhancements:
    ...
    def setup_method(self):
        ...
        self.get_option_bar = self.options_handler.get_option_bar
        self.get_options_chain = self.options_handler.get_options_chain
        self.strategy = CreditSpreadStrategy(get_contract_list_for_date=self.get_contract_list_for_date, get_option_bar=self.get_option_bar, get_options_chain=self.get_options_chain, lstm_model=self.lstm_model, lstm_scaler=self.lstm_scaler, symbol='SPY', options_handler=self.options_handler)
        dates = pd.date_range(start='2024-01-01', periods=100, freq='D')
        self.sample_data = pd.DataFrame({'Close': [100 + i * 0.5 for i in range(100)], 'Open': [100 + i * 0.5 for i in range(100)], 'High': [101 + i * 0.5 for i in range(100)], 'Low': [99 + i * 0.5 for i in range(100)], 'Volume': [1000000] * 100}, index=dates)
        self.strategy.set_data(self.sample_data)
    ...
    def test_get_current_volumes_for_position_success(self):
        ...
        position = create_position(symbol='SPY', expiration_date=datetime(2024, 1, 19), strategy_type=StrategyType.CALL_CREDIT_SPREAD, strike_price=450.0, entry_date=datetime(2024, 1, 15), entry_price=2.0, spread_options=[option1, option2])
        from algo_trading_engine.vo import StrikePrice, ExpirationDate
        contract1 = OptionContractDTO(ticker='O:SPY240119C00450000', underlying_ticker='SPY', contract_type=CommonOptionType.CALL, strike_price=StrikePrice(Decimal('450.0')), expiration_date=ExpirationDate(datetime(2024, 1, 19).date()), exercise_style='american', shares_per_contract=100)
        contract2 = OptionContractDTO(ticker='O:SPY240119C00455000', underlying_ticker='SPY', contract_type=CommonOptionType.CALL, strike_price=StrikePrice(Decimal('455.0')), expiration_date=ExpirationDate(datetime(2024, 1, 19).date()), exercise_style='american', shares_per_contract=100)
        bar1 = OptionBarDTO(ticker='O:SPY240119C00450000', timestamp=datetime(2024, 1, 16), open_price=Decimal('5.0'), high_price=Decimal('5.1'), low_price=Decimal('4.9'), close_price=Decimal('5.0'), volume=200, volume_weighted_avg_price=Decimal('5.0'), number_of_transactions=100)
        bar2 = OptionBarDTO(ticker='O:SPY240119C00455000', timestamp=datetime(2024, 1, 16), open_price=Decimal('3.0'), high_price=Decimal('3.1'), low_price=Decimal('2.9'), close_price=Decimal('3.0'), volume=250, volume_weighted_avg_price=Decimal('3.0'), number_of_transactions=80)

        def mock_get_option_with_bar(strike, expiry_date, option_type, _date):
    ...
    def test_get_current_volumes_for_position_no_volume_data(self):
        ...
        self.strategy.get_option_bar = Mock(return_value=None)
        test_date = datetime(2024, 1, 16)
        volumes = self.strategy.get_current_volumes_for_position(position, test_date)
        assert volumes == [None]
...
class TestDataRetrieverLivePrice:
    ...
    def test_get_live_price_caches_by_symbol_across_instances(self, mock_ticker):
        ...
        mock_ticker_instance.info = {'currentPrice': 450.0, 'regularMarketPrice': 450.0, 'previousClose': 445.0}
        mock_ticker.return_value = mock_ticker_instance
        retriever_one = DataRetriever(symbol='SPY')
        retriever_two = DataRetriever(symbol='SPY')
