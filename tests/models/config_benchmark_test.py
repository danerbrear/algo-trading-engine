...
class TestBacktestConfigBenchmarkTicker:
    ...
    def test_default_benchmark_ticker_is_none(self):
        ...
        config = BacktestConfig(initial_capital=100000, start_date=datetime(2024, 1, 1), end_date=datetime(2024, 1, 31), symbol='DIA', strategy_type='credit_spread')
    ...
    def test_benchmark_ticker_can_be_set(self):
        ...
        config = BacktestConfig(initial_capital=100000, start_date=datetime(2024, 1, 1), end_date=datetime(2024, 1, 31), symbol='DIA', strategy_type='credit_spread', benchmark_ticker='SPY')
    ...
    def test_benchmark_ticker_same_as_symbol(self):
        ...
        config = BacktestConfig(initial_capital=100000, start_date=datetime(2024, 1, 1), end_date=datetime(2024, 1, 31), symbol='SPY', strategy_type='credit_spread', benchmark_ticker='SPY')
        assert config.benchmark_ticker == 'SPY'
    ...
    def test_benchmark_ticker_is_immutable(self):
        ...
        with pytest.raises(Exception):
            config.benchmark_ticker = 'QQQ'

def _make_price_data(start, periods, base_price=100.0):
    """Helper to create a mock price DataFrame."""
    return pd.DataFrame({'Close': [base_price + i for i in range(periods)], 'Open': [base_price + i - 1 for i in range(periods)], 'High': [base_price + i + 1 for i in range(periods)], 'Low': [base_price + i - 2 for i in range(periods)], 'Volume': [1000000] * periods}, index=pd.date_range(start, periods=periods, freq='D'))
...
class TestBacktestEngineBenchmarkFromConfig:
    ...
    def test_no_benchmark_ticker_uses_trading_data(self, mock_create_strategy, _mock_options_handler, mock_data_retriever, _mock_ml_prep):
    ...
    @patch('algo_trading_engine.backtest.main.OptionsHandler')
    @patch('algo_trading_engine.backtest.main.create_strategy_from_args')
    def test_benchmark_ticker_same_as_symbol_skips_extra_fetch(self, mock_create_strategy, _mock_options_handler, mock_data_retriever, _mock_ml_prep):
    ...
    @patch('algo_trading_engine.backtest.main.OptionsHandler')
    @patch('algo_trading_engine.backtest.main.create_strategy_from_args')
    def test_different_benchmark_ticker_fetches_separate_data(self, mock_create_strategy, _mock_options_handler, mock_data_retriever, _mock_ml_prep):
    ...
    @patch('algo_trading_engine.backtest.main.OptionsHandler')
    @patch('algo_trading_engine.backtest.main.create_strategy_from_args')
    def test_benchmark_fetch_failure_falls_back(self, mock_create_strategy, _mock_options_handler, mock_data_retriever, _mock_ml_prep):
        ...
        def side_effect():
            ...
            return retriever
        mock_data_retriever.side_effect = lambda **kw: side_effect()
        config = BacktestConfig(initial_capital=100000, start_date=datetime(2024, 1, 1), end_date=datetime(2024, 1, 31), symbol='DIA', strategy_type='credit_spread', benchmark_ticker='INVALID')
        engine = BacktestEngine.from_config(config)
        assert engine.benchmark_data is None
...
class TestBacktestEngineBenchmarkRun:
    ...
    def test_benchmark_uses_separate_data_for_prices(self):
        ...
        trading_data = _make_price_data('2024-01-01', 5, base_price=200.0)
        benchmark_data = _make_price_data('2024-01-01', 5, base_price=400.0)
        engine = BacktestEngine(data=trading_data, strategy=mock_strategy, initial_capital=100000, start_date=datetime(2024, 1, 1), end_date=datetime(2024, 1, 5), enable_progress_tracking=False, quiet_mode=True, benchmark_data=benchmark_data)
        engine.run()
    ...
    def test_benchmark_falls_back_to_trading_data_when_no_benchmark_data(self):
        ...
        mock_strategy.get_warm_up_period_timedelta = Mock(return_value=timedelta(0))
        trading_data = _make_price_data('2024-01-01', 5, base_price=200.0)
        engine = BacktestEngine(data=trading_data, strategy=mock_strategy, initial_capital=100000, start_date=datetime(2024, 1, 1), end_date=datetime(2024, 1, 5), enable_progress_tracking=False, quiet_mode=True, benchmark_data=None)
        engine.run()
