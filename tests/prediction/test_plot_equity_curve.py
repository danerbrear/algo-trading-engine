...
import pandas as pd
from algo_trading_engine.prediction.plot_equity_curve import ClosedPosition, calculate_equity_curve, calculate_drawdowns, fetch_spy_data, fetch_treasury_rates, plot_equity_curve
...
class TestClosedPosition:
    ...
    def test_from_decision_dict_credit_spread(self):
        ...
        assert position.pnl == (2.5 - 1.0) * 1 * 100
...
class TestCalculateEquityCurve:
    ...
    def test_calculate_equity_curve_single_position(self):
        ...
        position = ClosedPosition(closed_at=closed_at, entry_price=2.5, exit_price=1.0, quantity=1, strategy_name='test', strategy_type='put_credit_spread', pnl=150.0)
    ...
    def test_calculate_equity_curve_multiple_positions(self):
        ...
        positions = [ClosedPosition(closed_at=datetime(2025, 11, 1, 15, 0, 0), entry_price=2.5, exit_price=1.0, quantity=1, strategy_name='test', strategy_type='put_credit_spread', pnl=150.0), ClosedPosition(closed_at=datetime(2025, 11, 5, 15, 0, 0), entry_price=3.0, exit_price=1.5, quantity=1, strategy_name='test', strategy_type='put_credit_spread', pnl=150.0), ClosedPosition(closed_at=datetime(2025, 11, 10, 15, 0, 0), entry_price=2.0, exit_price=3.0, quantity=1, strategy_name='test', strategy_type='put_credit_spread', pnl=-100.0)]
...
class TestFetchSpyData:
    ...
    def test_fetch_spy_data_success(self, mock_ticker):
        ...
        mock_data = pd.DataFrame({'Close': [450.0, 452.0, 455.0]}, index=pd.date_range('2025-11-01', periods=3))
...
class TestFetchTreasuryRates:
    ...
    def test_fetch_treasury_rates_success(self, mock_ticker):
        ...
        mock_data = pd.DataFrame({'Close': [4.25, 4.3, 4.28]}, index=pd.date_range('2025-11-01', periods=3))
        mock_ticker_instance = Mock()
        mock_ticker_instance.history.return_value = mock_data
        mock_ticker.return_value = mock_ticker_instance
        start_date = datetime(2025, 11, 1)
